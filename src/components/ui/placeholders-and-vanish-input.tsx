...
interface PlaceholdersAndVanishInputProps {
  ...
  value?: string; // Optional external value control
}

export const PlaceholdersAndVanishInput = forwardRef<
  HTMLFormElement,
  PlaceholdersAndVanishInputProps
>(({ placeholders, onChange, onSubmit, value: externalValue }, ref) => {
    ...
    const animateFrame = (pos: number = 0) => {
      requestAnimationFrame(() => {
        ...
        if (newDataRef.current.length > 0) {
          ...
          // Reset value based on control mode
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (!animating) {
      // Update internal value only if not externally controlled
      if (externalValue === undefined) {
